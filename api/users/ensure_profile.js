const { getUserFromJwt, serviceSelect, serviceInsert, serviceUpdate } = require('../_supabase');
...
module.exports = async (req, res) => {
  try {
    ...
    if (Array.isArray(existing.json) && existing.json[0]) {
      ...
      if (wantSuper && (!prof.team_id || String(prof.team_id).trim() === '')) patch.team_id = 'morning';
    ...
    const profileRow = {
      ...
      name,
      role,
      team_id: (role === 'SUPER_ADMIN') ? 'morning' : null,
      duty: duty || ''
    };
