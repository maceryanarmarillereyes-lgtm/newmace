...
// Notify same-tab listeners (used to keep UI in sync without reload).
  ...
  function sanitizeUsers(list){
    ...
    for(const raw of (Array.isArray(list) ? list : [])){
      ...
      // Requirement: Super Admin must have NO team/shift assignment.
      // - Keep teamId blank (do NOT coerce to a default team)
      // - Clear any schedule assignment
      if(u.role === 'SUPER_ADMIN'){
        u.teamId = '';
        if('schedule' in u) u.schedule = null;
      } else {
  ...
  const Store = {
    ensureSeed(){
      ...
      const hasMeys = users.find(u => String(u.username||'').toUpperCase()==='MUMS');

      // Seed policy:
      // - If this is a brand-new install (no users at all), create a single SUPER_ADMIN bootstrap account (MUMS).
      // - If users already exist and MUMS was intentionally deleted, do NOT resurrect it (prevents "deleted users reappearing").
      // - If MUMS exists, keep it SUPER_ADMIN but remove team/shift assignments per requirements.
      const mkMeys = (id)=>({
        id: id || uuid(),
        username: 'MUMS',
        email: 'meys@local',
        ...
        schedule: null,
        status: 'active',
        passwordHash: (window.Auth && typeof Auth.hash === "function") ? Auth.hash('MUMS') : 'h0',
        createdAt: Date.now(),
      });
      ...
      if(!out.length && !hasMeys){
        ...
        // Migrate MUMS to the new Super Admin policy (no team/shift assignment).
        out = out.map(u=>{
          if(u && u.id===hasMeys.id){
            const keepHash = (u.passwordHash && String(u.passwordHash).trim()) ? u.passwordHash : ((window.Auth && typeof Auth.hash === "function") ? Auth.hash('MUMS') : 'h0');
            return {
              ...u,
              username: 'MUMS',
              role: (window.Config ? Config.ROLES.SUPER_ADMIN : 'SUPER_ADMIN'),
