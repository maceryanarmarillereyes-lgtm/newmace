...
function renderWorldClocksBar(){
  ...
  // === Online Users Bar (3 team sections) ===
  function _bucketForUser(u){
    try{
      ...
      try{
        ...
        if ((!teamId || !role) && uid && window.Store && Store.getUserById) {
          ...
          if (su) {
            ...
            if (!role && su.role) role = String(su.role || '').toUpperCase();
      ...
      if(!teamId){
        // Super Admins may intentionally have no team assignment; pin them to Morning bucket for monitoring.
        if(role === 'SUPER_ADMIN' || role === 'SUPER_USER') return 'morning';
        return 'mid';
      }
