...
function renderWorldClocksBar(){
  ...
  function _bucketForUser(u){
    try{
      ...
      // Developer Access: always excluded from shift monitoring buckets
      if(role === 'SUPER_ADMIN' || role === 'SUPER_USER') return 'dev';
...
function updateClocksPreviewTimes(){
  ...
  function openProfileModal(user){
    ...
    if(teamEl) teamEl.value = (team && team.label) ? team.label : '';
    ...
    UI.el('#profileSave').onclick = async ()=>{
      ...
      if(cloudProfileEnabled()){
        const out = await CloudUsers.updateMe({ name: name || (user.name||user.username) });
        if(!out.ok){
      ...
      Store.updateUser(user.id, { name: name || user.username });
...
async function boot(){
  ...
  function bindSidebarToggle(){
    ...
    const closeTemp = ()=>{
      ...
      try{ if(cleanupTemp){ cleanupTemp(); cleanupTemp=null; } }catch(_){}
      try{ applySidebarState(); }catch(_){}
    };

    const openTemp = ()=>{
      // Temporarily expand sidebar without changing persisted collapsed/pinned state.
      closeTemp();
      try{
      ...
      const onDocDown = (e)=>{
        try{
          ...
          if(e && e.target && side.contains(e.target)) return;
          closeTemp();
        }catch(_){}
      };
      const onKey = (e)=>{ try{ if(e.key==='Escape') closeTemp(); }catch(_){ } };
      const onLeave = ()=>{ try{ leaveTimer = setTimeout(()=>closeTemp(), 180); }catch(_){ } };
      const onEnter = ()=>{ try{ if(leaveTimer){ clearTimeout(leaveTimer); leaveTimer=null; } }catch(_){ } };
      ...
      cleanupTemp = ()=>{
        ...
        try{ side.removeEventListener('mouseleave', onLeave); }catch(_){}
        try{ side.removeEventListener('mouseenter', onEnter); }catch(_){}
        try{ if(leaveTimer){ clearTimeout(leaveTimer); leaveTimer=null; } }catch(_){}
      };
    };

    const setPinned = (val)=>{
      localStorage.setItem('mums_sidebar_pinned', val ? '1' : '0');
      if(val){
        // Pinned means: keep sidebar collapsed (rail mode) persistently.
        localStorage.setItem('mums_sidebar_collapsed', '1');
      }
      closeTemp();
      applySidebarState();
    };
    ...
    const setCollapsed = (val)=>{
      localStorage.setItem('mums_sidebar_collapsed', val ? '1' : '0');
      if(!val){
        // Expanding clears pin (explicit user intent).
        localStorage.setItem('mums_sidebar_pinned', '0');
      }
      closeTemp();
      applySidebarState();
    };
    ...
    const handleSingle = ()=>{
      const pinned = (localStorage.getItem('mums_sidebar_pinned')||'0')==='1';
      const collapsed = document.body.classList.contains('sidebar-collapsed') || (localStorage.getItem('mums_sidebar_collapsed')||'0')==='1';

      if(pinned){
        // When pinned, single click provides a temporary peek without changing pin state.
        if(document.body.classList.contains('sidebar-tempopen')) closeTemp();
        else openTemp();
        return;
      }

      // One click -> auto collapse / expand (persisted)
      setCollapsed(!collapsed);
    };

    const handleDouble = ()=>{
      // Double click -> pin sidebar (toggle)
      const pinned = (localStorage.getItem('mums_sidebar_pinned')||'0')==='1';
      if(!pinned){
        setPinned(true);
      }else{
        // Unpin and expand for clarity
        localStorage.setItem('mums_sidebar_pinned', '0');
        setCollapsed(false);
      }
    };
