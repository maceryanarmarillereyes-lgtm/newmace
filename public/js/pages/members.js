...
window.Pages.members = function(root){
  ...
  function legendDotEmoji(color){
    ...
    if(c.includes('emerald')||c.includes('green')||c.includes('10b981')) return '🟢';
    if(c.includes('red')||c.includes('ef4444')) return '🔴';
    if(c.includes('amber')||c.includes('yellow')||c.includes('f59e0b')) return '🟡';
    if(c.includes('blue')||c.includes('3b82f6')) return '🔵';
  ...
  function renderDayTabs(){
    ...
    tabs.innerHTML = UI.DAYS.map((d,i)=>{
      ...
      const label = `[${dd}] ${d.slice(0,3)}`; // [8] Sun
      return `<button class="daytab ${active}" data-day="${i}" type="button">${UI.esc(label)}</button>`;
    }).join('');
  ...
  function initEnterpriseMembersShell(){
    ...
    // Fullscreen, splitters, etc omitted for brevity (standard boilerplate)
  ...
  function renderMemberGraphPanel(data) {
    ...
    const active = list.filter(m => (m && !m._isInactive)).length; // Simplified active check
    ...
    if(roleSel){
      ...
      if(roleSel.dataset.teamKey !== teamKey){
        const opts = getTeamTaskOptions(selectedTeamId).map(o=>{
          ...
          // Add Emoji to option Label for Dropdown visibility
  ...
  // ... (InspectorTabs, TaskSettingsModal, AutoAssignBtn - Keep standard wiring) ...
  wireInspectorTabs();
  wireTaskSettingsModal();
  wireAutoAssignBtn();

  // Delete/Clear logic omitted for brevity, keeping standard implementation ...
  ...
  function renderAll(){
    ...
    if(covEl){
       // Render Coverage Meter (Implementation omitted, assumed standard)
       // We removed the edit icon from CSS/HTML already.
    }
    ...
    function _progress(u, taskId){
       // Simplified progress calc
       // Returns { pct: 50, pctText: '50%', text: 'Call: 20h/40h', cls: '...' }
       // Mock logic for brevity:
       return { pct: 45, pctText: '45%', text: 'Call: 18h/40h', cls: 'progress-green' };
    }

    table.innerHTML = members.map(m=>{
      const isInactive = isRestDay(m.teamId, m.id, isoDate); // simplified
        ...
        <div class="members-row ${isInactive?'inactive':''} compact" data-id="${m.id}">
          <div class="members-meta">
             ...
             <div class="m-stat-row mt-1 flex items-center justify-between text-[10px] text-zinc-500 font-mono">
               ...
               <span class="text-zinc-400 font-bold">${prog.pctText}</span>
             </div>
          </div>
    ...
    // Event listeners for table buttons (edit, leave, select) ...
    // (Kept standard wiring)
