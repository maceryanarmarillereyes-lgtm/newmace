...
return <section class="task-section"> <div class="task-section-title">📥 My Assigned Tasks</div> ${state.assignedGroups.length ? state.assignedGroups.map(renderAssignedCard).join('') : '<div class="task-empty">No assigned tasks yet</div>'} </section>;
}
...
return <section class="task-section"> <div class="task-section-title">📌 ONGOING (Active Batches)</div> ${ongoing.length ? ongoing.map(renderDistributionCard).join('') : '<div class="task-empty">No ongoing distributions</div>'} </section> <section class="task-section"> <details> <summary class="task-section-title">✅ COMPLETED (100% Done)</summary> <div style="display:flex;flex-direction:column;gap:10px;margin-top:10px"> ${completed.length ? completed.map(renderDistributionCard).join('') : '<div class="task-empty">No completed distributions</div>'} </div> </details> </section>;
}
...
return <div class="task-modal-backdrop" id="problemModalBackdrop"> <div class="task-modal" role="dialog" aria-modal="true"> <div style="display:flex;justify-content:space-between;align-items:center;gap:10px"> <h3>${esc(title)}</h3> <button class="task-btn task-btn-ghost" type="button" id="problemModalClose">Close</button> </div> <div style="margin-top:10px"> <textarea id="problemModalNotes" rows="4" placeholder="Enter problem details..." style="width:100%">${esc(state.problemModal.notes || '')}</textarea> ${err ?${esc(err)}: ''} </div> <div class="task-modal-actions" style="margin-top:14px"> <button class="task-btn task-btn-ghost" type="button" id="problemModalCancel">Cancel</button> <button class="task-btn task-btn-primary" type="button" id="problemModalSave">Save</button> </div> </div> </div>;
}

function renderLoginAlertModal() {
const info = state.loginAlert || {};
return <div class="task-modal-backdrop" id="loginAlertBackdrop"> <div class="login-alert-modal" role="dialog" aria-modal="true"> <div class="login-alert-pill">⚠️ High Priority Login Alert</div> <div style="font-size:22px;font-weight:900;line-height:1.2">Overdue tasks require immediate action</div> <div class="login-alert-summary"> You have <strong>${esc(String(info.overdueCount || 0))}</strong> overdue task(s).<br> Distribution: ${esc(info.distributionTitle)}<br> Deadline: ${esc(info.nearestDeadline)} </div> <div class="login-alert-actions"><button class="task-btn task-btn-primary" type="button" id="loginAlertAcknowledge">Acknowledge</button></div> </div> </div>;
}

function render() {
ensureStyleTag();
root.innerHTML = <section class="task-shell"> ${state.loading || state.creating ? '<div class="task-overlay"><div class="task-spinner"></div></div>' : ''} <header class="task-header"> <h2 class="task-title-main">My Task</h2> <nav class="task-tabs"> <button type="button" class="task-tab ${state.activeTab === 'assigned' ? 'active' : ''}" id="tabAssigned">📥 My Assigned Tasks</button> <button type="button" class="task-tab ${state.activeTab === 'distribution' ? 'active' : ''}" id="tabDistribution">🚀 Distribution Management</button> <button type="button" class="btn primary" id="openDistributionModal" ${state.activeTab === 'distribution' ? '' : 'style="display:none"'}>+ Create Distribution</button> </nav> </header> ${state.activeTab === 'assigned' ? renderAssignedPanel() : renderDistributionPanel()} ${state.modalOpen ? renderModal() : ''} ${state.problemModal && state.problemModal.open ? renderProblemModal() : ''} ${state.loginAlert && state.loginAlert.open ? renderLoginAlertModal() : ''} </section>;
bindEvents();
