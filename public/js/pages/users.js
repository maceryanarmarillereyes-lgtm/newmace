  ...
  function openUserModal(actor, user){
    ...
    UI.el('#btnSaveUser').onclick = async ()=>{
      ...
      if(user){
        ...
        // Cloud mode: create user via Supabase (server-side) so login works immediately.
        const isCloud = !!(window.CloudAuth && CloudAuth.isEnabled && CloudAuth.isEnabled() && window.CloudUsers && typeof CloudUsers.create === 'function');

        if (isCloud) {
          // Enforce canonical email format so users can log in using either username or email.
          ...
          const canonicalEmail = `${username}@${domain}`;
          if (email && String(email).toLowerCase() !== String(canonicalEmail).toLowerCase()) {
            return err(`Email must match ${canonicalEmail} (or leave blank) so users can log in via username.`);
          }
          ...
          // Reload roster from Supabase to ensure UI reflects authoritative cloud roster (no duplicates).
          try { await CloudUsers.refreshIntoLocalStore(); } catch(_) {}
      ...
      UI.closeModal('userModal');
      // Refresh roster so the newly created user is shown immediately
