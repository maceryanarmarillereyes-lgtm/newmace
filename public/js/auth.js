  ...
  const Auth = {
    ...
    async login(identifier, password) {
      ...
      if (window.CloudAuth && CloudAuth.isEnabled && CloudAuth.isEnabled()) {
        ...
        // Prefer the enriched profile-backed user for UI permissions.
        try {
          const enriched = (window.Store && Store.getUserById) ? Store.getUserById(out.user.id) : null;
          currentUser = enriched || out.user;
        } catch (_) {
