...
async function loadHandler(env) {
  ...
  if (!mod) {
    const loaded = await import('../../server/routes/quickbase_tabs.js');
...
export async function onRequest(context) {
  ...
  try {
    const params = context.params && context.params.tab_id ? { tab_id: context.params.tab_id } : {};
    await handler(nodeReq, nodeRes, params);
