...
// 2. Fetch ALL items (FIX: Explicitly Quote UUIDs for PostgREST)
const distIds = dists.map((d) => d.id).filter(Boolean);
const quotedIds = distIds.map(id => `"${id}"`).join(',');
const tOut = await serviceSelect('task_items', `select=id,distribution_id,assigned_to,status,case_number,case_no,site&distribution_id=in.(${quotedIds})`);
