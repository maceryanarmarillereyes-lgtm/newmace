...
module.exports = async (req, res) => {
  try {
    ...
    // Force Super status for Supervisory roles
    ...
    // 2. BULLETPROOF FETCH: Concurrent 'eq.' queries to bypass 'in.' filter bugs
    const itemPromises = distIds.map(async (id) => {
      const out1 = await serviceSelect(
        'task_items',
        `select=id,distribution_id,assigned_to,status,case_number,case_no,site&distribution_id=eq.${id}`
      );
      let batchItems = out1.ok && Array.isArray(out1.json) ? out1.json : [];

      // Fallback to legacy column if empty
      if (batchItems.length === 0) {
        const out2 = await serviceSelect(
          'task_items',
          `select=id,task_distribution_id,assigned_to,status,case_number,case_no,site&task_distribution_id=eq.${id}`
        );
        if (out2.ok && Array.isArray(out2.json)) {
          batchItems = out2.json.map((item) => ({ ...item, distribution_id: item.task_distribution_id }));
        }
      }
    ...
    const profileMap = {};
    if (userIds.length) {
      // Safe to use 'in' for profiles as user_id is often simple string or manageable UUID array
      const pOut = await serviceSelect('mums_profiles', `select=user_id,name,username,team_id&user_id=in.(${userIds.join(',')})`);
      if (pOut.ok) (pOut.json || []).forEach((p) => {
